(function (root, factory) {
  ...
  var handleTest = function (success, result, msg) {
    if (!success) {
      result.valid = false;
      result.msg = msg;
    }
  ...
  var nonEmpty = function (v, rules) {
    return function (customMsg) {
      ...
      rules.push(function (result, xs) {
        return handleTest(xs.length > 0, result, msg);
      });
      return v;
    };
  };
  ...
  var simpleTest = function (test, defaultMsg) {
    return function (v, rules) {
      return function (customMsg) {
        ...
        rules.push(function (result, x) {
          return handleTest(test(x), result, msg);
        });
        return v;
      };
    };
  ...
  var runner = function (rules, makeResultSchema) {
    return function (x, keepResult) {
      var init = makeResultSchema ? makeResultSchema() : {};
      init.valid = true;
      return rules.reduce(function (chain, rule) {
        return chain.then(function (result) {
          return BPromise.resolve(rule(result, x));
        });
      }, BPromise.resolve(init)).then(function (result) {
  ...
  var create = function () {
    ...
    v.date = mkV(function (v, rules) {
      return function (customMsg) {
        ...
        rules.push(function (result, x) {
          return handleTest(isDate(x), result, msg);
        });
        return v;
      };
    }, {
      valid: function (v, rules) {
        return function (customMsg) {
          ...
          rules.push(function (result, x) {
            return handleTest(!isNaN(x.getTime()), result, msg);
          });
          return v;
        };
      }
      ...
      v.prop = function (name, xv) {
        rules.push(function (result, x) {
          if (!hasOwnProp.call(x, name)) {
            return handleTest(false, result,
