...
var runner = function (rules, resultSchema) {
  return function (x, keepResult) {
    var result = _.assign({ valid: true }, resultSchema);
...
var mkV = function (test, namedTests) {
  var create = function (customMsg) {
    ...
    _.each(create.tests, function (fn, k) {
      v[k] = fn(v, rules);
    });
  ...
  create.tests = _.assign({}, namedTests);
  return create;
};
